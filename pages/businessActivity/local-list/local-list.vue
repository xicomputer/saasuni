<template>
    <!-- pages/businessActivity/local-list/local-list.wxml -->
                    ...
                    <block v-for="(item, index1) in currentList" :key="index1">
                        <view :class="'order-item order-' + item.orderNumber">
                            ...
                            <view class="receive-goods">
                                ...
                                <view v-if="item.shippingAddress.contacts">收货人: {{ item.shippingAddress.contacts + ' ' + item.shippingAddress.contactWay }}</view>
                                <view v-if="item.shippingAddress.provincesName">
                                    收货地址:
                                    {{ item.shippingAddress.provincesName + item.shippingAddress.cityName + item.shippingAddress.areaName + item.shippingAddress.address }}
                                </view>
                                <view v-if="currentState == 2">{{ item.expressCompany }}：{{ item.expressNo }}</view>
                                <view v-if="currentState == 2">商家发货时间：{{ item.deliveryTime }}</view>
                            </view>
                        </view>
                        ...
                        <view class="order-item-foolt">
                            ...
                            <view class="me-fx-row me-fx-end-c foot-btns">
                                <text class="confirm-btn contact-btn" @tap="toChat" :data-item="item">联系商家</text>
...
<style>
/* pages/businessActivity/local-list/local-list.wxss */
