    ...
    <view>
        <view>
            ...
            <view class="order-list" v-if="dataList.length">
                <view class="order-item" :data-item="item" @tap="jumpDetail" v-for="(item, index) in dataList" :key="index">
                    ...
                    <view class="me-fx-row me-fx-end-c item-footer">
                        ...
                        <button class="btn first-btn" :data-item="item" @tap.stop.prevent="openWhyList" v-if="(item.orderState == 4 || item.orderState == 1) && item.isEnd != 1">
                            {{ item.activityOrderRefundResponse ? '查看售后' : '申请退款' }}
                        </button>

                        <button class="btn last-btn" v-if="item.orderState == 0 && item.isDelete != 1" :data-item="item" @tap.stop.prevent="goPay">去支付</button>
                        <button class="btn last-btn" v-if="item.orderState == 1" :data-item="item" @tap.stop.prevent="remindDelivery">提醒发货</button>
