<template>
    <view>
        <!-- pages/seckill/booking-list/booking-list.wxml -->
            ...
            <view class="order-list" v-if="dataList.length">
                <view class="order-item" :data-item="item" v-for="(item, index) in dataList" :key="index">
                    ...
                    <view class="me-fx-row me-fx-end-c item-footer">
                        ...
                        <!-- <button class='btn first-btn' wx:if='{{item.orderState==1 }}' data-item='{{item}}' catchtap='contactMerchant'>确认核销</button> -->
                        <button class="btn first-btn" v-if="item.isDelete == 1 || item.orderState == 0 || item.orderState == 4" :data-item="item" @tap.stop.prevent="delOrder">
                            删除订单
                        </button>

                        <button class="btn last-btn" v-if="item.orderState == 0 && item.isDelete != 1" :data-item="item" @tap.stop.prevent="goPay">去支付</button>
                        <button class="btn last-btn" v-if="item.orderState == 1" :data-item="item" @tap.stop.prevent="showVerificationCode">核销码</button>
...
<style>
/* pages/seckill/booking-list/booking-list.wxss */
