<template>
    <view>
        <!-- pages/seckill/local-list/local-list.wxml -->
            ...
            <view class="order-list" v-if="dataList.length">
                <view class="order-item" :data-item="item" v-for="(item, index) in dataList" :key="index">
                    ...
                    <view class="me-fx-row me-fx-end-c item-footer">
                        ...
                        <button class="btn first-btn" v-if="item.orderState == 2 || item.orderState == 4" :data-item="item" @tap.stop.prevent="contactMerchant">联系商家</button>
                        <button class="btn first-btn" v-if="item.isDelete == 1 || item.orderState == 0 || item.orderState == 3" :data-item="item" @tap.stop.prevent="delOrder">
                            删除订单
                        </button>
                        <!-- <button class='btn first-btn' data-item='{{item}}' catchtap='openWhyList' wx:if='{{item.orderState==0 && item.isDelete!=1}}'>取消订单</button> -->
                        <!-- <button class='btn first-btn' data-item='{{item}}' catchtap='openWhyList' wx:if='{{(item.orderState==4 || item.orderState==1) && item.isEnd!=1 }}'>{{item.activityOrderRefundResponse?'查看售后':'申请退款'}}</button> -->

                        <button class="btn last-btn" v-if="item.orderState == 0 && item.isDelete != 1" :data-item="item" @tap.stop.prevent="goPay">去支付</button>

                        <button class="btn last-btn" v-if="item.orderState == 2" :data-item="item" @tap.stop.prevent="confirmDelivery">确认收货</button>
                        <!-- <button class='btn last-btn' wx:if='{{item.orderState==4}}' data-item='{{item}}'  catchtap='jumpEvaluate'>去评价</button> -->
                        <button class="btn last-btn" v-if="item.isDelete == 1 || item.orderState == 3" :data-item="item" @tap.stop.prevent="jumpGoodsDetail">重新购买</button>
                    </view>
                </view>
            </view>
...
<style>
/* pages/seckill/local-list/local-list.wxss */
...
.order-list .item-footer .first-btn {
    border: 2rpx solid #999;
    color: #999;
    background-color: #fff;
}

.order-list .item-footer .last-btn {
