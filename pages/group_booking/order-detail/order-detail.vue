    ...
    <view>
        <view>
            <view class="me-fx-row me-fx-sb-c top-box" v-if="orderInfo.isDelete == 1 || orderInfo.orderState == 3">
                <image src="/static/pages/group_booking/imgs/yqx.png" class="adorn-img"></image>
                ...
                <view class="me-fx-1">
                    ...
                    <block v-if="orderState == 2">
                        <van-count-down
                            ...
                            :use-slot="true"
                            :auto-start="true"
                            @finish="finishFun($event, { timename: 'downTimeObj' })"
                            @change="changeFun($event, { timename: 'downTimeObj' })"
                            data-timename="downTimeObj"
                        >
                            <view class="sub-title">
                                <text>剩余</text>
                                <text v-if="downTimeObj.days !== '0'">{{ downTimeObj.days }}天</text>
            ...
            <block v-if="orderInfo.isDelete != 1">
                ...
                <view class="me-fx-row me-fx-sb-c logistics-info" @tap="jumpLogisticsShow" v-if="orderInfo.expressNo">
                    <image src="/static/pages/group_booking/imgs/wl.png" class="wl-icon-img"></image>
                    ...
                    <image src="/static/pages/group_booking/imgs/right-jiantou.png" class="jiantou-img"></image>
                </view>
            ...
            <!-- 拼团信息 -->
            <view class="group-info" v-if="orderInfo.activityGroupResponse">
                <view class="me-fx-row me-fx-sb">
                    <view>
                        <view>{{ groupInfo.nickname }}的拼团</view>
                        <view class="poor-num">
                            差
                            <text>{{ groupInfo.groupOfNumber - joinGroupUser.length }}</text>
                            人成团
                        </view>
                    </view>
                    <van-count-down
                        ...
                        :use-slot="true"
                        :auto-start="true"
                        @finish="finishFun($event, { timename: 'groupTimeObj' })"
                        @change="changeFun($event, { timename: 'groupTimeObj' })"
                        data-timename="groupTimeObj"
                    >
                        <view class="group-down-str">
                            <text v-if="groupTimeObj.days !== '00'">{{ groupTimeObj.days }}天</text>
                            <text>{{ groupTimeObj.hours }}:{{ groupTimeObj.minutes }}:{{ groupTimeObj.seconds }} 后结束</text>
                        </view>
                    </van-count-down>
                </view>
                <view class="me-fx-row me-fx-start-c user-list">
                    <view class="avatar-item" v-for="(item, index) in joinGroupUser" :key="index">
                    ...
                    <image
                        ...
                        v-for="(item, ti) in groupInfo.groupOfNumber - joinGroupUser.length"
                        :key="item.ti"
                    ></image>
                </view>
            </view>
            ...
            <view class="me-fx-row me-fx-end-c footer-btns">
                ...
                <button class="btn blue-btn" v-if="orderState == 1 || orderState == 2 || orderState == 4" @tap="contactMerchant">联系商家</button>
                <button class="btn red-btn" :data-item="item" @tap.stop.prevent="openWhyList" v-if="(orderState == 4 || orderState == 1) && orderInfo.isEnd != 1" data-type="2">
                ...
                <button class="btn blue-btn" v-if="orderInfo.isDelete == 1 || orderInfo.orderState == 3 || orderInfo.orderState == 0" @tap="delOrder">删除订单</button>

                <button class="btn red-btn" v-if="orderState == 0 && orderInfo.isDelete != 1" @tap="goPay">去支付</button>
                <button class="btn yellow-btn" v-if="orderState == 2" @tap="confirmDelivery">确认收货</button>
                <!-- <button class='btn solid-btn' wx:if='{{orderState==4}}' bindtap='jumpEvaluate'>去评价</button> -->
                <button class="btn yellow-btn" v-if="orderInfo.isDelete == 1 || orderInfo.orderState == 3" @tap="jumpGoodsDetail">再次拼单</button>
                <button class="btn yellow-btn" v-if="orderState == 1" @tap="remindDelivery">提醒发货</button>
            </view>
        </view>
...
.group-info .poor-num text {
    color: #ffbb38;
}

.group-info .user-list {
    padding: 20rpx 0;
}
.group-info .user-list .avatar-item {
    position: relative;
    margin-right: 20rpx;
}
.group-info .user-list .avatar-item .user-avatar {
    width: 100rpx;
    height: 100rpx;
    border-radius: 50%;
    background-color: #ccc;
}
.group-info .user-list .avatar-item .initiator-user-label {
...
.group-info .user-list .empty-avatar-img {
    width: 100rpx;
    height: 100rpx;
    margin-right: 20rpx;
}
...
.footer-btns .btn:not(:last-child) {
    margin-right: 15rpx;
}
...
.footer-btns .blue-btn {
    background-color: #1577ff;
}
.footer-btns .yellow-btn {
